...
async function main() {
    const productsRes = await request("1. GET /products", "/products");
    ...
    await request("2. GET /products/:id", `/products/${firstProductId}`);
    ...
    const orderRes = await request("7. POST /orders", "/orders", {
        method: "POST",
        body: JSON.stringify({
            paymentMethod: "COD",
        }),
